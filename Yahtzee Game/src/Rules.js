...
class Rule {
  ...
  freq(dice) {
    ...
    console.log("Freqss = " + Array.from(freqs.values())
                + "Dices..=" + JSON.stringify(dice) 
                + "  Join= " + JSON.stringify(Array.from(freqs.values()))

    );
    console.log(Array.from(freqs.values()).includes(1,2));
...
class FullHouse extends Rule{
  // TODO
  evalRoll = dice => {
    // let freqD = this.freq(dice);
    // if ((JSON.stringify(freqD) === JSON.stringify([2,3])) || (JSON.stringify(freqD) === JSON.stringify([3,2]))){
    // return this.score; } else {
    //   return 0;
    // }
